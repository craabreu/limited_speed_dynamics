...
respa_system.redefine_bond(pdb.topology, 'HOH', 'H[1-2]', 'O', 1.03119*unit.angstroms)
respa_system.redefine_angle(pdb.topology, 'HOH', 'H[1-2]', 'O', 'H[1-2]', 107.672*unit.degrees)

loops = [6, max(1, args.timestep//3), 1]

integrator = atomsmm.integrators.SIN_R_Integrator(dt, loops, temp, tau, gamma, L=args.L)
...
computer = atomsmm.PressureComputer(openmm_system,
    pdb.topology,
    platform if args.secdev is None else openmm.Platform.getPlatformByName('CUDA'),
    properties if args.secdev is None else dict(Precision='mixed', DeviceIndex=args.secdev),
    temperature=temp,
)
